...
def init_args():
    ...
    parser.add_argument("--do_eval", action='store_true', # create flag = False -> declare -> True
...
def train(args, train_dataset, model, tokenizer):
    ...
    train_sampler = RandomSampler(train_dataset) if args.local_rank == -1 else DistributedSampler(train_dataset) # for each epoch
...
def main():
    ...
    for i in range(1, n_times):
        ...
        precision = results['precision_%s' % step]
        recall = results['recall_%s' % step]
